...
describe('PATCH events route', () =>{
    ...
    test('Rejects patch-all request (status 405): Method not allowed.', 
        async() =>{
            const response = await request(app).patch('/events').send(data.validData)
    ...
    test('Responds with Validation Errors (status 400): Invalid reference Id', 
        async() =>{
            const response = await request(app).patch('/events/64c9e4f2df7cc072af2acxx')
                .send(data.validData)
    ...
    test('Responds with validation errors (status 400): Invalid input', 
        async() =>{
            const response = await request(app).patch('/events/64c9e4f2df7cc072af2ac9e4')
    ...
    test('Responds with Not Found(404): event does not exist', 
        async() =>{
            const response = await request(app).patch('/events/64c9e4f2df7cc072af2ac8a4')
                .send(data.validData)
